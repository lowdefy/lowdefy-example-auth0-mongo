...
function MongoDBAdapter({ properties }) {
  ...
  return {
    ...
    async updateUser(data) {
      const { _id, inviteId, roles, attributes, ...user } = data;
      user.updated = { timestamp: new Date(), user: { id: user._id } };
      const result = await (
        await db
    ...
    linkAccount: async (account) => {
      await (await db).accounts.insertOne({ ...account, _id: id(account) });
      return from(account);
    },
    ...
    async createSession(session) {
      await (await db).sessions.insertOne({ ...session, _id: id(session) });
      return session;
    },
    ...
    async createVerificationToken(data) {
      await (await db).verificationTokens.insertOne({ ...data, _id: id(data) });
      return data;
    },
